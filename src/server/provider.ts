...
export const openAIProvider: InterviewProvider = {
  async followUp(input, context, signal) {
    const result = await aiClient().responses.parse(
      {
        model: process.env.FEEDBACK_MODEL ?? "gpt-5-mini",
        store: false,
        max_output_tokens: 2500,
        instructions:
          instructions +
          " Ask exactly one concise technical follow-up grounded in a specific point from answer1. If the answer lacks detail, ask for an example rather than assuming one. Do not answer your own question.",
        input: JSON.stringify({ context, answer1: input.answer1 }),
        text: { format: zodTextFormat(questionOutput, "follow_up") },
      },
      { signal },
    );
    if (!result.output_parsed) throw new Error("No structured follow-up.");
  ...
  async feedback(input, context, signal) {
    ...
    if (!result.output_parsed) throw new Error("No structured review.");
    return result.output_parsed;
  },
};
