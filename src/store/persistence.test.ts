...
it("restores a draft without restoring an in-flight request", async () => {
  ...
  const a = createTrainerStore(storage);
  await a.persist.rehydrate();
  a.getState().start(context, false);
  a.getState().editAnswer(1, "A retained lexical binding.");
  a.getState().confirmAnswer();
  a.getState().beginRequest("follow-up");
  const b = createTrainerStore(storage);
  expect(b.getState().session).toBeNull();
  await b.persist.rehydrate();
  expect(b.getState().session?.answer1).toBe("A retained lexical binding.");
  expect(b.getState().request).toBeNull();
  expect(b.getState().hydrated).toBe(true);
});
it("reports corrupted storage instead of silently treating it as a saved session", async () => {
  const storage = memory();
  storage.setItem("eye-practice", '{"version":1,"state":{"session":"broken"}}');
  const store = createTrainerStore(storage);
  await store.persist.rehydrate();
  await Promise.resolve();
  expect(store.getState().storageError).toMatch(/saved|storage|data/i);
...
it("reports write failure while retaining the current answer in memory", async () => {
  ...
  await Promise.resolve();
  expect(store.getState().session?.context.title).toBe("Closures");
